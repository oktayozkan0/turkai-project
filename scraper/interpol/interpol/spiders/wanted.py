...
class WantedSpider(scrapy.Spider):
    ...
    def start_requests(self):
        yield scrapy.Request("https://www.interpol.int/How-we-work/Notices/View-Red-Notices", callback=self.nationalities)

    def nationalities(self, response):
        data = response.xpath("//select[@id='arrestWarrantCountryId']/option/@value").getall()
        for d in data:
            yield scrapy.Request(f"https://ws-public.interpol.int/notices/v1/red?arrestWarrantCountryId={d}&&resultPerPage=160&page=1", callback=self.paging_if_necessary)
    ...
    def paging_if_necessary(self, response):
        ...
        for d in data["_embedded"]["notices"]:
            ...
            try:
                ...
                item["thumbnail"] = None
            item["url"] = f"https://www.interpol.int/How-we-work/Notices/View-Red-Notices#{d.get('entity_id').replace('/','-')}"
            yield item
